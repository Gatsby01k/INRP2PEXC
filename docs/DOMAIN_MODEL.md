...
erDiagram
  ...
  FIAT_TRANSFER ||--o{ TRANSFER_ALLOCATION : "one movement"
  CRYPTO_TRANSFER ||--o{ TRANSFER_ALLOCATION : "one movement"
  TRANSFER_ALLOCATION }o--o| SETTLEMENT_LEG : "CLIENT dimension"
  TRANSFER_ALLOCATION }o--o| ROUTE_SETTLEMENT : "ROUTE dimension"
  TRANSFER_ALLOCATION }o--o| EXCEPTION_CASE : "CLIENT dimension (unattributed)"
  FIAT_TRANSFER ||--o| LEDGER_JOURNAL : "fiat:{f}:confirm"
  CRYPTO_TRANSFER ||--o| LEDGER_JOURNAL : "crypto:{x}:confirm"
...
|---|---|---|
| `user`, `session`, `account`, `verification`, `two_factor` | Managed by **Better Auth** (schema generated by Better Auth and committed as explicit SQL migrations). INRP2P additional fields on `user`: kind (`OPERATOR`,`CLIENT`), status (`ACTIVE`,`DISABLED`); on `session`: surface (`CLIENT`,`OPERATOR`) | Email verification state (Better Auth `emailVerified`) is the "verified contact" signal for D-01. Credentials and TOTP secrets are stored only as Better Auth manages them |
...
| `step_up_verification` | user_id, session_id, method (`TOTP`), verified_at, ip_hash | INRP2P record written after a successful Better Auth TOTP verification; commands read it for step-up freshness (10 min). Enrolled TOTP is required for every operator |
...
| `client_user` | client_id, user_id, role (`CLIENT_ADMIN`,`CLIENT_TRADER`), can_accept_quotes (bool, audited), status — "verified client contact" for D-01 = `user.email` of a client_user with `can_accept_quotes` and an email verified by Better Auth |
...
| Entity | Key fields |
|---|---|
| `liquidity_route` | name (internal only), direction, asset, network, status, settlement_model (`PER_TRADE` · `PREFUNDED` · `NET_SETTLED`; V1 validation accepts only `PER_TRADE`), execution_mode (`DIRECT_TO_CLIENT`: route delivers the client's payout asset straight to the client's bank/wallet · `TO_EXCHANGE`: route delivers to exchange accounts/treasury; default for new trades, frozen per trade), registered_payout_identity (operator-only; e.g. expected remitter name/account last4 for direct INR), registered_route_address (USDT), available_base_minor (operator-maintained), notes. **Never** exposed to client APIs |
| `route_obligation` | ref, route_id, trade_id (unique while model = `PER_TRADE`; nullable for future netting), direction, exchange_delivers_asset, exchange_delivers_minor, route_delivers_asset, route_delivers_minor, execution_mode (frozen), status (`OPEN`,`PARTIALLY_SETTLED`,`SETTLED`,`CANCELLED`), opened_at (= acceptance), settled_at — amounts copied from `trade_economics` at acceptance and never updated; remaining per side is derived from allocations and equals the ledger balance of that obligation's route lines (FI-64). SELL: exchange delivers `base` USDT, route delivers `route_value_inr`. BUY: exchange delivers `route_value_inr` INR, route delivers `base` USDT |
| `route_settlement` | ref, route_id, flow (`FROM_ROUTE_TO_EXCHANGE`,`TO_ROUTE`,`DIRECT_TO_CLIENT`), asset, amount_minor (= linked movement amount), adapter (`MANUAL`), adapter_reference, status (`RECORDED`,`CONFIRMED`,`FAILED`), created_by (`SYSTEM` for `DIRECT_TO_CLIENT`), confirmed_at, notes — references exactly one movement through `transfer_allocation` (dimension ROUTE). `DIRECT_TO_CLIENT` rows are created only by the direct payout confirm and share the movement of a client payout leg |
...
| `trade_economics` | trade_id (PK), base_minor, quote_inr_minor, client_rate_micro, route_rate_micro, route_value_inr_minor, gross_margin_inr_minor, route_id, route_execution_mode (frozen from route at acceptance), fees_json (explicit fee lines, V1 empty), network, bank_account_id, crypto_wallet_id, frozen_at | Insert-once |
...
| payer | `CLIENT` (first leg) · `EXCHANGE_ACCOUNT` · `ROUTE` (payout leg in `DIRECT_TO_CLIENT` mode only) |
| route_id | Set when payer = `ROUTE` |
| inr_account_id / capacity_reservation_id | Only when payer = `EXCHANGE_ACCOUNT` and asset INR (CHECK) |
...
|---|---|
| `fiat_transfer` | **one real INR movement**: rail, utr, utr_normalized, amount_minor, payer_type (`CLIENT`,`EXCHANGE_ACCOUNT`,`ROUTE`) + payer_id, payee_type (`CLIENT_BANK`,`EXCHANGE_ACCOUNT`,`ROUTE`) + payee_id, destination (masked snapshot), value_date, status (`RECORDED`,`CONFIRMED`,`FAILED`), recorded_by, confirmed_at, proof_attachment_id, ledger_journal_id | Unique `(rail, utr_normalized)`; journal key `fiat:{id}:confirm` |
| `crypto_transfer` | **one real USDT movement**: network, tx_hash, log_index, payer_type/payee_type (resolved from known addresses), token_contract, from_address, to_address, amount_minor, block_number, block_time, detected_at, receipt_status, state (`DETECTED`,`CONFIRMED`,`FAILED`,`ORPHANED`), confirmed_at, source (`SCANNER`,`OPERATOR_SUBMITTED`) | Unique `(network, tx_hash, log_index)`; journal key `crypto:{id}:confirm` |
| `transfer_allocation` | transfer_kind (`FIAT`,`CRYPTO`), transfer_id, dimension (`CLIENT`,`ROUTE`), settlement_leg_id · exception_case_id (CLIENT) or route_settlement_id (ROUTE), amount_minor (= transfer amount), allocated_by (`SYSTEM` or user) — unique `(transfer_kind, transfer_id, dimension)`; both dimensions on one movement only when payer = `ROUTE` and payee = client (FI-28). Over/short payments are handled by exceptions, never by splitting one movement. Client deposits are allocated only through the trade's deposit assignment (D-02) |
...
| `ROUTE_DIRECT_PAYOUT_MISMATCH` | Direct payout confirm rejected: amount exceeds route obligation remaining, route mismatch, or trade not in `DIRECT_TO_CLIENT` mode | yes | `reclassify_payer` (if the exchange actually paid) · `financial_adjustment` · `record_correct_route` |
...
- Added `acceptance_challenge` for OTP-verified quote-link acceptance and rejection (`DECISIONS.md D-01`) — an INRP2P domain primitive, separate from Better Auth.
- Replaced per-evidence allocation tables with one `transfer_allocation` over `fiat_transfer`/`crypto_transfer`, so a single direct route→client movement can satisfy a client payout leg and a route obligation without duplicate evidence or postings (`DECISIONS.md D-14`).
- Identity tables for users, sessions, accounts, verifications and two-factor secrets are owned by Better Auth (`DECISIONS.md D-06`); INRP2P adds operator roles, client users and permissions around them.
