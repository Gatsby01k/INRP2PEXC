...
| T10 | any non-terminal with confirmed funds → CANCELLED | `exception.resolve(refund_and_cancel)` — DEALER initiates, FINANCE approves (two-person) | Refund leg(s) COMPLETED for all confirmed client funds; no payout PROCESSING or COMPLETED (else adjustment path) | releases all reservations; PENDING legs cancelled | refund movement journals + `trade:{t}:cancel` + `adj:{id}:cancel` | `trade.cancelled` | posting keys | stays in state until refunds confirm |
...
A trade pays out **or** refunds, never both at once (FI-25): a planned or completed refund stops payout creation, sending and confirmation; a payout that is PROCESSING or COMPLETED stops refund creation, confirmation and `refund_and_cancel`.
