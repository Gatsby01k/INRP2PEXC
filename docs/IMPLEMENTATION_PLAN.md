...
Desk (strip + grouped queue + context panels), Orders, Rates (incl. Route positions), `TO_EXCHANGE` route settlement via `ManualRouteAdapter` (FINANCE/OWNER): record against one obligation side, then confirm ⧗ — the confirmation posts the movement journal and allocates that side atomically, and there is no separate allocation action in V1 (`STATE_MACHINES.md §9`, Phase 4 above). Direct route payout legs in the payout panel (payer selector, route-reported sent, UTR, confirm) appear on Rates as read-only allocations, since confirming the client leg already allocated the route side (FI-64). INR, USDT (incl. deposit pool status), Clients (create quote, repeat trade), trade operations, command bar, keyboard flows, step-up dialogs.
